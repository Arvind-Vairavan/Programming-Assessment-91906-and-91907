"""
CAREER WINDOW v1 - Initial implementation with basic job display.

WHAT HAS BEEN DONE:
- Created CareerWindow with job info display
- Added Find Job, Work, Promote, Resign buttons
- Created JobSelectWindow for browsing available jobs
- Added job card display with emojis
- Added scrollable job list

WHAT WORKS:
- Job information displays correctly
- Buttons appear and respond to clicks
- Job selection window opens
- Jobs can be selected from list
...
HOW TO FIX IN V2:
- Fix experience display to show actual years
- Ensure consistent experience increment between Work and Age Up
...
class CareerWindow(BaseWindow):
    ...
    def _setup(self):
        ...
        # BUG: job_experience is stored in months but displayed as years with //
        # This floors the value, losing months of experience
        ...
        # Buttons row - BUG: May not expand properly on small windows
    ...
    def _job_card(self, inner, char, emojis):
        # BUG: Experience displayed with //12 again - inconsistent
...
class JobSelectWindow(BaseWindow):
    ...
    def _select(self, name):
        char = self.game.get_character()
        if char:
            # BUG: Experience resets to 0 when changing jobs
            # This may be intended but could be improved
            char.job = name
