"""
BASE WINDOW v1 - Initial implementation with basic window features.

WHAT HAS BEEN DONE:
- Created BaseWindow class with __init__ method
- Added add_title() for window headers with separator
- Added add_close() for standard close buttons
- Added card() for styled content grouping
- Added scrollable() for scrollable content areas with mousewheel

WHAT WORKS:
- Window creation with dark background
- Title display with separator
...
HOW TO FIX IN V2:
- Add _setup_styles() method to define all styles
- Fix mousewheel binding to be per-window instead of global
- Add platform detection for scrolling
"""
